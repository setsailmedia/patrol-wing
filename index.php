    ...
    try{
      ...
      this.ws.onclose=()=>{
        ...
        if(gameState==='playing'&&players.length>1){
          weaponFlash={prefix:'',name:'CONNECTION LOST — CONTINUING SOLO',ms:3000};
          const idx=players.indexOf(this.peerPlayer);
...
NET._onMsg=function(msg){
  ...
  if(msg.event==='client-launch'&&gameState==='waitingRoom'&&!NET.isHost){
    ...
    if(lm==='pvp'){pvpRound=0;pvpScore=[0,0];startPvP();}
      ...
      loadCustomLevel({
        name:'CTF Arena',worldW:3200,worldH:2400,winCondition:'captureFlag',winParams:{},
        spawnX:400,spawnY:1200,
        obstacles:[
          {type:'wall',x:1500,y:400,w:26,h:600},{type:'wall',x:1500,y:1400,w:26,h:600},
          {type:'pillar',x:800,y:600,r:40},{type:'pillar',x:800,y:1800,r:40},
          {type:'pillar',x:2400,y:600,r:40},{type:'pillar',x:2400,y:1800,r:40},
        ],
        enemies:[{type:'scout',x:1600,y:600},{type:'scout',x:1600,y:1800},{type:'guard',x:1600,y:1200}],
        pickups:[{type:'health',x:1600,y:1000,hidden:false},{type:'health',x:1600,y:1400,hidden:false},{type:'weapon',x:800,y:1200,hidden:false},{type:'weapon',x:2400,y:1200,hidden:false}],
        hazards:[],
        objectives:[
          {type:'flag_red',x:200,y:1200},{type:'base_red',x:200,y:1200},
          {type:'flag_blue',x:3000,y:1200},{type:'base_blue',x:3000,y:1200},
        ],
      });
    }
  ...
  if(msg.event==='client-gs'&&!NET.isHost){
    const d=msg.data;
    if(!d)return;
    ...
    if(d.en){
      ...
      for(let i=0;i<d.en.length;i++){
        if(i>=enemies.length){
          enemies.push({x:0,y:0,vx:0,vy:0,aim:0,hp:100,maxHp:100,type:'scout',size:13,color:'#ff2244',accent:'#ff9aaa',state:'patrol',stunMs:0,stunMoveMs:0,stunFireMs:0,infected:false,lastFired:0,fromHarbinger:false,_visible:true,_appearAt:0,_duration:0});
        }
        const e=enemies[i],s=d.en[i];
        e.x=s.x;e.y=s.y;e.aim=s.aim;e.hp=s.hp;e.maxHp=s.maxHp;e.type=s.type;
        e.size=s.size;e.color=s.color;e.accent=s.accent;e.state=s.state;e.stunMs=s.stunMs;
      }
    }
    if(d.pb){
      pBullets.length=0;
      for(const b of d.pb)pBullets.push({x:b.x,y:b.y,vx:b.vx,vy:b.vy,bSz:b.bSz,color:b.col,dmg:0,stun:false,dinf:false,fromInfected:false,isGrapple:false,life:500});
    }
    if(d.eb){
      eBullets.length=0;
      for(const b of d.eb)eBullets.push({x:b.x,y:b.y,vx:b.vx,vy:b.vy,dmg:10,bSz:3,isBrute:false,fromInfected:false,isBomb:false,life:1000});
    }
    if(d.sc!==undefined)score=d.sc;
    if(d.wv!==undefined)wave=d.wv;
    if(d.gs&&d.gs!==gameState){
      if(d.gs==='gameover')gameState='gameover';
      if(d.gs==='victory')gameState='victory';
      if(d.gs==='waveClear')gameState='waveClear';
    }
...
function _mpHostBroadcast(){
  ...
  const state={
    t:'state',
    hp:{x:P.x,y:P.y,aim:P.aim,hp:P.hp,bat:P.bat,alive:P.alive,vx:P.vx,vy:P.vy,shieldMs:P.shieldMs,invincMs:P.invincMs,cloakMs:P.cloakMs,overchargeMs:P.overchargeMs,rotor:P.rotor,iframes:P.iframes,weaponIdx:P.weaponIdx,kills:P.kills},
    en:enemies.slice(0,40).map(e=>({x:e.x,y:e.y,aim:e.aim,hp:e.hp,maxHp:e.maxHp,type:e.type,size:e.size,color:e.color,accent:e.accent,state:e.state,stunMs:e.stunMs||0})),
    sc:score,wv:wave,gs:gameState,
...
function _mpGuestSendInput(firing){
  if(!NET.connected||NET.isHost)return;
  NET.sendJSON('input',{
    x:P.x,y:P.y,aim:P.aim,vx:P.vx,vy:P.vy,rotor:P.rotor,
    alive:P.alive,hp:P.hp,weaponIdx:P.weaponIdx,
    fire:firing||false,
  });
}
...
let ctfCapturesToWin=3;
let ctfCarryingFlag=-1; // -1=not carrying, 0=carrying red flag, 1=carrying blue flag
let ctfFlagPositions=[null,null]; // current world positions of [red,blue] flags (null=at base)
let ctfRoundOverMs=0;
  ...
  for(const h of hazards){
    ...
    if(h.type==='zap_pylon'){
      ...
      for(let b=0;b<3;b++){
        ...
        const bx=sx+Math.cos(ba*1.3)*br,by2=sy+Math.sin(ba*0.9)*br;
...
function drawPlayer(){
  ...
  // Revive indicator
  if(mpRevivePos&&NET.connected){
    const rx=mpRevivePos.x-camX,ry=mpRevivePos.y-camY;
    const pulse=0.5+0.5*Math.sin(Date.now()/200);
    ctx.strokeStyle=`rgba(0,255,136,${0.4+0.3*pulse})`;ctx.lineWidth=2;
    ctx.beginPath();ctx.arc(rx,ry,MP_REVIVE_RANGE,0,Math.PI*2);ctx.stroke();
    ctx.font='bold 12px "Courier New"';ctx.fillStyle=`rgba(0,255,136,${0.7+0.3*pulse})`;
    ctx.textAlign='center';ctx.fillText('REVIVE',rx,ry-MP_REVIVE_RANGE-8);
    if(mpReviveMs>0){
      const pct=mpReviveMs/MP_REVIVE_TIME;
      ctx.strokeStyle='#00ff88';ctx.lineWidth=4;
      ctx.beginPath();ctx.arc(rx,ry,MP_REVIVE_RANGE+6,-Math.PI/2,-Math.PI/2+pct*Math.PI*2);ctx.stroke();
    }
    ctx.textAlign='left';
  }
}
...
function tickEnemies(dt,now){
  if(NET.connected&&!NET.isHost)return; // guest gets enemies from host
  for(const e of enemies){
...
function checkCollisions(){
  if(NET.connected&&!NET.isHost)return; // guest gets collision results from host
...
function drawHUD(){
  ...
  if(NET.connected&&players.length>1){
    ...
    if(p2){
      ...
      if(!p2.alive){
        ...
        ctx.textAlign='right';ctx.fillText('PARTNER DOWN — FLY TO REVIVE',canvas.width-pad,p2Y+bGap+bH+18);
      }
      ctx.textAlign='left';
    }
  }
  ...
  if(customWinCondition==='captureFlag'){
    ctx.textAlign='center';ctx.font='bold 18px "Courier New"';
    const rCol='#ff4444',bCol='#4488ff';
    ctx.fillStyle=rCol;ctx.fillText(`${ctfScores[0]}`,canvas.width/2-30,pad+14);
    ctx.fillStyle='rgba(200,200,200,0.6)';ctx.fillText('-',canvas.width/2,pad+14);
    ctx.fillStyle=bCol;ctx.fillText(`${ctfScores[1]}`,canvas.width/2+30,pad+14);
    ctx.font='10px "Courier New"';ctx.fillStyle='rgba(200,200,200,0.5)';
    ctx.fillText(`FIRST TO ${ctfCapturesToWin}`,canvas.width/2,pad+30);
    if(ctfCarryingFlag>=0){
      ctx.font='bold 14px "Courier New"';
      ctx.fillStyle=ctfCarryingFlag===0?rCol:bCol;
      ctx.shadowBlur=10;ctx.shadowColor=ctx.fillStyle;
      ctx.fillText('FLAG CARRIER -- RETURN TO BASE',canvas.width/2,80);
      ctx.shadowBlur=0;
    }
    ctx.textAlign='left';
  }
...
function drawMinimap(){
  ...
  // CTF flags on minimap
  if(customWinCondition==='captureFlag'){
    for(const obj of customObjectives){
      if((obj.type==='flag_red'||obj.type==='flag_blue')&&!obj.collected){
        const fx=ctfFlagPositions[obj.type==='flag_red'?0:1];
        const fmx=fx?fx.x:obj.x,fmy=fx?fx.y:obj.y;
        ctx.fillStyle=obj.type==='flag_red'?'#ff4444':'#4488ff';
        ctx.fillRect(mx+fmx*scx-2,my+fmy*scy-2,5,5);
      }
    }
  }
  // Portal markers
...
function spawnWave(n){
  if(NET.connected&&!NET.isHost)return; // guest gets wave data from host
...
function drawDeathScreen(){
  ...
  // Buttons
  const bw=220,bh=44,bx=cx-bw/2,gap=14;
...
function tickCustomWinCondition(dt){
  ...
  } else if(customWinCondition==='captureFlag'){
    if(ctfRoundOverMs>0){ctfRoundOverMs-=dt*1000;return;}
    const flagRed=customObjectives.find(o=>o.type==='flag_red');
    const flagBlue=customObjectives.find(o=>o.type==='flag_blue');
    const baseRed=customObjectives.find(o=>o.type==='base_red');
    const baseBlue=customObjectives.find(o=>o.type==='base_blue');
    if(!flagRed||!flagBlue||!baseRed||!baseBlue)return;
    const frx=ctfFlagPositions[0]?ctfFlagPositions[0].x:flagRed.x;
    const fry=ctfFlagPositions[0]?ctfFlagPositions[0].y:flagRed.y;
    const fbx=ctfFlagPositions[1]?ctfFlagPositions[1].x:flagBlue.x;
    const fby=ctfFlagPositions[1]?ctfFlagPositions[1].y:flagBlue.y;
    if(P.alive&&ctfCarryingFlag<0){
      if(!flagBlue.collected&&dist(P.x,P.y,fbx,fby)<40){
        ctfCarryingFlag=1;flagBlue.collected=true;
        ctfFlagPositions[1]={x:P.x,y:P.y};
        P.spd*=JRR_CARRY_SPD;
        weaponFlash={prefix:'',name:'FLAG CAPTURED -- RETURN TO BASE',ms:2500};
        SFX.pickup();
      }
    }
    if(ctfCarryingFlag>=0&&P.alive){
      ctfFlagPositions[ctfCarryingFlag]={x:P.x,y:P.y};
      if(ctfCarryingFlag===1&&dist(P.x,P.y,baseRed.x,baseRed.y)<50){
        ctfScores[0]++;
        ctfCarryingFlag=-1;
        ctfFlagPositions[1]=null;
        flagBlue.collected=false;
        P.spd=CRAFTS[P.craftIdx].spd;
        spawnParts(baseRed.x,baseRed.y,'#ff4444',_pCount(25),5,7,600);
        weaponFlash={prefix:'',name:`RED SCORES! ${ctfScores[0]}-${ctfScores[1]}`,ms:2500};
        SFX.confirm();
        if(ctfScores[0]>=ctfCapturesToWin) customLevelWin();
      }
    }
    if(!P.alive&&ctfCarryingFlag>=0){
      ctfFlagPositions[ctfCarryingFlag]={x:P.x,y:P.y};
      ctfCarryingFlag=-1;
      P.spd=CRAFTS[P.craftIdx].spd;
      weaponFlash={prefix:'',name:'FLAG DROPPED',ms:1800};
    }
    if(ctfFlagPositions[0]&&P.alive&&dist(P.x,P.y,ctfFlagPositions[0].x,ctfFlagPositions[0].y)<40){
      ctfFlagPositions[0]=null;
      flagRed.collected=false;
      weaponFlash={prefix:'',name:'FLAG RETURNED',ms:1500};
      SFX.select();
    }
  }
}
...
function drawCustomObjectives(){
  ...
  for(const obj of customObjectives){
    ...
    } else if(obj.type==='flag_red'&&!obj.collected){
      const pulse=0.5+0.5*Math.sin(t*3);
      ctx.save();ctx.translate(sx,sy);
      ctx.shadowBlur=12*pulse;ctx.shadowColor='#ff4444';
      ctx.strokeStyle='#aaaaaa';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(0,12);ctx.lineTo(0,-12);ctx.stroke();
      ctx.fillStyle=`rgba(255,68,68,${0.8+0.2*pulse})`;ctx.beginPath();ctx.moveTo(0,-12);ctx.lineTo(14,-6);ctx.lineTo(0,0);ctx.closePath();ctx.fill();
      ctx.shadowBlur=0;ctx.restore();
    } else if(obj.type==='flag_blue'&&!obj.collected){
      const pulse=0.5+0.5*Math.sin(t*3+1);
      ctx.save();ctx.translate(sx,sy);
      ctx.shadowBlur=12*pulse;ctx.shadowColor='#4488ff';
      ctx.strokeStyle='#aaaaaa';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(0,12);ctx.lineTo(0,-12);ctx.stroke();
      ctx.fillStyle=`rgba(68,136,255,${0.8+0.2*pulse})`;ctx.beginPath();ctx.moveTo(0,-12);ctx.lineTo(14,-6);ctx.lineTo(0,0);ctx.closePath();ctx.fill();
      ctx.shadowBlur=0;ctx.restore();
    } else if(obj.type==='base_red'){
      const pulse=0.4+0.3*Math.sin(t*2);
      const r=50+4*pulse;
      ctx.beginPath();ctx.arc(sx,sy,r,0,Math.PI*2);
      ctx.strokeStyle=`rgba(255,68,68,${0.35+0.2*pulse})`;ctx.lineWidth=3;
      ctx.shadowBlur=14*pulse;ctx.shadowColor='#ff4444';ctx.stroke();ctx.shadowBlur=0;
      ctx.font='10px "Courier New"';ctx.fillStyle='rgba(255,100,100,0.7)';ctx.textAlign='center';
      ctx.fillText('RED BASE',sx,sy+r+14);
    } else if(obj.type==='base_blue'){
      const pulse=0.4+0.3*Math.sin(t*2+1);
      const r=50+4*pulse;
      ctx.beginPath();ctx.arc(sx,sy,r,0,Math.PI*2);
      ctx.strokeStyle=`rgba(68,136,255,${0.35+0.2*pulse})`;ctx.lineWidth=3;
      ctx.shadowBlur=14*pulse;ctx.shadowColor='#4488ff';ctx.stroke();ctx.shadowBlur=0;
      ctx.font='10px "Courier New"';ctx.fillStyle='rgba(100,150,255,0.7)';ctx.textAlign='center';
      ctx.fillText('BLUE BASE',sx,sy+r+14);
...
function drawLevelSetup(){
  ...
  const conditions=[
    ...
    {id:'captureFlag',label:'CAPTURE FLAG',desc:'Grab flag, return to base'},
  ];
...
function _getEditorCategories(){
  return [
    ...
    {id:'objective',label:'OBJECTIVES',items:[
      ...
      ...(editorWinCondition==='captureFlag'?[
        {tool:'obj_flag_red',label:'Flag Red',cat:'objective',subtype:'flag_red'},
        {tool:'obj_flag_blue',label:'Flag Blue',cat:'objective',subtype:'flag_blue'},
        {tool:'obj_base_red',label:'Base Red',cat:'objective',subtype:'base_red'},
        {tool:'obj_base_blue',label:'Base Blue',cat:'objective',subtype:'base_blue'},
      ]:[]),
    ]},
...
function drawLevelEditor(){
  ...
  for(const item of editorPlacedItems){
    ...
    if(item.cat==='obstacle'&&item.subtype==='gate'){
      ...
      else if(item.subtype==='flag_red'){
        ctx.strokeStyle='#aaa';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(sx,sy+10);ctx.lineTo(sx,sy-10);ctx.stroke();
        ctx.fillStyle='#ff4444';ctx.beginPath();ctx.moveTo(sx,sy-10);ctx.lineTo(sx+12,sy-5);ctx.lineTo(sx,sy);ctx.closePath();ctx.fill();
        ctx.font='8px "Courier New"';ctx.fillStyle='#ff6666';ctx.textAlign='center';ctx.fillText('FLAG R',sx,sy+20);
      } else if(item.subtype==='flag_blue'){
        ctx.strokeStyle='#aaa';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(sx,sy+10);ctx.lineTo(sx,sy-10);ctx.stroke();
        ctx.fillStyle='#4488ff';ctx.beginPath();ctx.moveTo(sx,sy-10);ctx.lineTo(sx+12,sy-5);ctx.lineTo(sx,sy);ctx.closePath();ctx.fill();
        ctx.font='8px "Courier New"';ctx.fillStyle='#6699ff';ctx.textAlign='center';ctx.fillText('FLAG B',sx,sy+20);
      } else if(item.subtype==='base_red'){
        ctx.strokeStyle='rgba(255,68,68,0.5)';ctx.lineWidth=2;ctx.beginPath();ctx.arc(sx,sy,30,0,Math.PI*2);ctx.stroke();
        ctx.font='8px "Courier New"';ctx.fillStyle='#ff6666';ctx.textAlign='center';ctx.fillText('BASE R',sx,sy+38);
      } else if(item.subtype==='base_blue'){
        ctx.strokeStyle='rgba(68,136,255,0.5)';ctx.lineWidth=2;ctx.beginPath();ctx.arc(sx,sy,30,0,Math.PI*2);ctx.stroke();
        ctx.font='8px "Courier New"';ctx.fillStyle='#6699ff';ctx.textAlign='center';ctx.fillText('BASE B',sx,sy+38);
      }
    }
    if(item.appearAfter>0||item.duration>0){
...
function _doClick(){
  ...
  if(gameState==='levelSetup'){
    ...
    const conditions=['killAll','reachFinish','survive','retrieve','collectAll','captureFlag'];
    const condW=(panelW-2*8)/3,condH=52;
    for(let i=0;i<conditions.length;i++){
      const row=i<3?0:1,col=i<3?i:i-3;
      const rowW=3;
  ...
  if(gameState==='gameover'){
    const cx=canvas.width/2,bw=220,bh=44,bx=cx-bw/2,gap=14;
    if(gameMode==='custom'&&customPack){
      const by0=canvas.height/2+112,by1=by0+bh+gap,by2=by1+bh+gap;
      if(mouse.x>bx&&mouse.x<bx+bw&&mouse.y>by0&&mouse.y<by0+bh){loadCustomLevel(customPack.levels[customPack.currentIdx]);return;}
      if(mouse.x>bx&&mouse.x<bx+bw&&mouse.y>by1&&mouse.y<by1+bh){deathScreenEnteredAt=0;gameState='customSelect';SFX.select();return;}
      if(mouse.x>bx&&mouse.x<bx+bw&&mouse.y>by2&&mouse.y<by2+bh){_returnToStart();SFX.select();return;}
      return;
    }
    const by=canvas.height/2+112;
...
function _iGradBg(){
  ...
  for(let s=0;s<30;s++){
    const sx2=((s*137.5+t*20*(1+s%3*0.5))%canvas.width);
    const sy2=((s*89.3+t*8*(1+s%2))%canvas.height);
    const sz=0.5+((s*31)%3)*0.5;
    const sa=0.15+((s*17)%5)*0.05;
    ctx.beginPath();ctx.arc(sx2,sy2,sz,0,Math.PI*2);
    ctx.fillStyle=`rgba(150,200,255,${sa})`;ctx.fill();
  }
...
function _drawIntroSeq0(){
  ...
  if(dA>0){
    ...
    const entryDur=800; // ms per craft entry
    const crafts=[
      {draw:drawPhantom,name:'PHANTOM',col:'#00ccff',tx:cx-fsp,ty:fy,sz:34,spn:8,delay:0},
      {draw:drawViper,name:'VIPER',col:'#ff4400',tx:cx+fsp,ty:fy,sz:26,spn:11,delay:300},
      {draw:drawTitan,name:'TITAN',col:'#ff8800',tx:cx-fsp*0.38,ty:fy+105,sz:28,spn:7,delay:600},
      {draw:drawSpecter,name:'SPECTER',col:'#aa44ff',tx:cx+fsp*0.38,ty:fy+105,sz:24,spn:10,delay:900},
    ];
    crafts.forEach((c2,i)=>{
      const elapsed=ms-500-c2.delay;
      if(elapsed<0)return;
      const progress=Math.min(1,elapsed/entryDur);
      const easeOut=1-Math.pow(1-progress,3);
      const startX=-100-i*60;
      const fx=startX+(c2.tx-startX)*easeOut;
      const fy2=c2.ty+Math.sin(t*0.8+i*1.1)*8;
      c2.draw(fx,fy2,Math.PI*0.5+Math.sin(t*0.3+i*0.5)*0.12,c2.sz,c2.col,lighten(c2.col),t*c2.spn);
      // Engine trail
      if(progress<1){
        for(let tr=0;tr<4;tr++){
          const ta=0.4+0.6*Math.random();
          ctx.beginPath();ctx.arc(fx-15-tr*8-Math.random()*10,fy2+(Math.random()-0.5)*8,2+Math.random()*2,0,Math.PI*2);
          ctx.fillStyle=c2.col;ctx.globalAlpha=dA*(1-progress)*ta*0.5;ctx.fill();
        }
        ctx.globalAlpha=dA;
      }
      // Label
      if(progress>0.8){
        ctx.globalAlpha=dA*Math.min(1,(progress-0.8)*5);
        ctx.textAlign='center';ctx.font='8px "Courier New"';ctx.fillStyle=c2.col;
        ctx.shadowBlur=8;ctx.shadowColor=c2.col;ctx.fillText(c2.name,fx,fy2+(i<2?-46:-16+40));ctx.shadowBlur=0;
      }
    });
    ctx.globalAlpha=dA;
    // Connection lines between crafts (fade in after all arrived)
    const allIn=ms>500+900+entryDur;
    if(allIn){
      const lineA=_iFadeIn(ms,500+900+entryDur,500);
      ctx.globalAlpha=dA*lineA;
      ctx.strokeStyle='rgba(0,190,255,0.07)';ctx.lineWidth=1;ctx.setLineDash([4,10]);
      const pts=crafts.map(c2=>({x:c2.tx,y:c2.ty+Math.sin(t*0.8)*8}));
      [[0,1],[0,2],[1,3]].forEach(([a,b])=>{ctx.beginPath();ctx.moveTo(pts[a].x,pts[a].y);ctx.lineTo(pts[b].x,pts[b].y);ctx.stroke();});
      ctx.setLineDash([]);
    }
    ctx.restore();
  }
...
function _drawIntroSeq2(){
  ...
  // Screen shake
  const shk=Math.sin(t*17)*2*Math.min(1,ms/2000);
  ctx.save();ctx.translate(shk*(Math.random()-0.5),shk*(Math.random()-0.5));
  const mA=_iFadeIn(ms,0,600);
  ctx.globalAlpha=mA;
  const offX=Math.min(230,canvas.width*0.28);
  // Friendly craft — dynamic zigzag movement
  const p1x=cx-offX+Math.sin(t*1.2)*40,p1y=cy-20+Math.cos(t*0.9)*35;
  const p2x=cx-offX-70+Math.sin(t*1.0+2)*30,p2y=cy+55+Math.cos(t*0.8+1)*25;
  drawPhantom(p1x,p1y,Math.atan2(cy-p1y,cx+offX-p1x),28,'#00ccff',lighten('#00ccff'),t*9);
  drawViper(p2x,p2y,Math.atan2(cy-p2y,cx+offX-p2x)-0.2,20,'#ff4400',lighten('#ff4400'),t*12);
  // Enemy craft — aggressive movement
  const e1x=cx+offX+Math.sin(t*1.3+1)*35,e1y=cy-20+Math.cos(t*1.1+0.5)*30;
  const e2x=cx+offX+75+Math.sin(t*0.9+3)*25,e2y=cy+55+Math.cos(t*0.7+2)*20;
  const e3x=cx+offX-30+Math.sin(t*1.1)*20,e3y=cy-80+Math.cos(t*0.8+4)*25;
  drawEnemyDrone(e1x,e1y,Math.PI+Math.sin(t*0.5)*0.3,15,'#ff2244','#ff9aaa',t*9);
  drawEnemyDrone(e2x,e2y,Math.PI*1.12+Math.sin(t*0.4)*0.2,14,'#ff2244','#ff9aaa',-t*8);
  drawEnemyDrone(e3x,e3y,Math.PI*0.92+Math.cos(t*0.6)*0.25,17,'#ff8800','#ffcc44',t*7);
  // Bullets — flying between the two sides with varied trajectories
  for(let i=0;i<16;i++){
    const phase=((t*1.2+i*0.07)%1),fromR=(i%2===0);
    const srcX=fromR?p1x+20:e1x-20;
    const srcY=fromR?(i%4<2?p1y:p2y):(i%3===0?e1y:i%3===1?e2y:e3y);
    const dstX=fromR?(cx+offX):(cx-offX);
    const bxp=srcX+(dstX-srcX)*phase;
    const byp=srcY+(cy-srcY)*phase*0.3+Math.sin(t*2+i)*8;
    const col=fromR?'#00eeff':'#ffaa22';
    const sz=fromR?2.5:2;
    ctx.beginPath();ctx.arc(bxp,byp,sz,0,Math.PI*2);
    ctx.fillStyle=col;ctx.shadowBlur=8;ctx.shadowColor=col;ctx.fill();ctx.shadowBlur=0;
  }
  // Explosions — random bursts
  for(let e=0;e<3;e++){
    const et=(t*0.7+e*1.3)%2.5;
    if(et<0.4){
      const ex=cx+(Math.sin(e*7.3)*offX*0.8),ey=cy+(Math.cos(e*5.1)*60);
      const ep=et/0.4;
      const er=10+ep*25;
      ctx.beginPath();ctx.arc(ex,ey,er,0,Math.PI*2);
      ctx.fillStyle=`rgba(255,${150+Math.floor(ep*100)},0,${(1-ep)*0.4})`;ctx.fill();
      ctx.beginPath();ctx.arc(ex,ey,er*0.6,0,Math.PI*2);
      ctx.fillStyle=`rgba(255,255,200,${(1-ep)*0.5})`;ctx.fill();
    }
  }
  // VS divider with pulsing
  const vsPulse=0.5+0.5*Math.sin(t*4);
  ctx.strokeStyle=`rgba(255,255,255,${0.04+0.04*vsPulse})`;ctx.lineWidth=1;ctx.setLineDash([6,12]);
  ctx.beginPath();ctx.moveTo(cx,cy-170);ctx.lineTo(cx,cy+130);ctx.stroke();ctx.setLineDash([]);
  ctx.textAlign='center';ctx.font='bold 22px "Courier New"';ctx.fillStyle=`rgba(255,255,255,${0.12+0.08*vsPulse})`;
  ctx.fillText('VS',cx,cy-25);
  ctx.restore(); // undo shake
  const nA=_iFadeIn(ms,0,600);
...
function _drawIntroSeq6(){
  ...
  if(fA>0){
    ...
    [[cx,fY-30,'#00ccff'],[cx+sp*0.52,fY+28,'#ff4400'],[cx-sp*0.52,fY+28,'#ff8800'],[cx,fY+90,'#aa44ff']].forEach(([fx,fy2,c])=>{
      for(let k=0;k<8;k++){
        const ta=t*3+k*0.4;
        const tx2=fx+(Math.sin(ta)*3),ty2=fy2+38+k*10+Math.sin(ta*1.5)*2;
        const sz=3-k*0.3;
        ctx.beginPath();ctx.arc(tx2,ty2,Math.max(0.5,sz),0,Math.PI*2);
        ctx.fillStyle=c;ctx.globalAlpha=fA*(1-k/8)*0.55;ctx.shadowBlur=6;ctx.shadowColor=c;ctx.fill();ctx.shadowBlur=0;
      }
    });
    ctx.restore();
  }
...
// ── Intro dispatcher ──────────────────────────────────────────
function drawIntro(){
  const fns=[_drawIntroSeq0,_drawIntroSeq1,_drawIntroSeq2,_drawIntroSeq3,_drawIntroSeq5,_drawIntroSeq6];
  ...
  if(gameState==='intro'){
    ...
    if(NET.connected){
      ...
      if(NET.isHost){
        ...
        if(now2-NET.lastSendTime>NET.SEND_RATE){
          const shooting=mouse.down||(IS_TOUCH&&touchSticks.R.active&&Math.sqrt(touchSticks.R.dx**2+touchSticks.R.dy**2)/STICK_R>STICK_DEAD);
          _mpGuestSendInput(shooting);
          NET.lastSendTime=now2;
        }
      }
    }
    ...
    if(NET.connected&&players.length>1){
      ...
      if(dead&&alive&&!mpRevivePos){
        mpRevivePos={x:dead.x||P.x,y:dead.y||P.y};
      }
      if(mpRevivePos&&alive){
        if(dist(alive.x,alive.y,mpRevivePos.x,mpRevivePos.y)<MP_REVIVE_RANGE){
          ...
          mpReviveMs=Math.max(0,mpReviveMs-dt*500); // decay if not in range
        }
      }
    }
